...
public class EventClient {
    ...
    public EventClient(RapidApiProperties rapidApiProperties, RestTemplate restTemplate, ClientResponseService clientResponseService) {
    ...
    public EventResponse findEvents(Integer fixtureId) {
        ...
        if (byRequest.isPresent()) {
            try {
                EventResponse fixtureResponse = objectMapper.readValue(byRequest.get().getResponse(), EventResponse.class);
        ...
        try {
            ...
            throw new ApiSportsException(String.format("Event not found by fixture id %s\nError: %s", fixtureId, e.getMessage()));
        }
    }
}
